...
def run_matchup(agent1_func, name1, agent2_func, name2, num_games=20):
    ...
    for g in range(num_games):
        # alterna a ordem de quem começa para eliminar vies do 1º turno
        if g % 2 == 0:
...
def main():
    ...
    # 1. rule-based vs random
    w1, t1 = run_matchup(rule_agent, "Rule-Based", random_agent, "Random", MATCHES_PER_PAIR)
    results.append({"Confronto": "Rule-Based vs Random", "Vitórias P1": f"Rule-Based ({w1['Rule-Based']})", "Vitórias P2": f"Random ({w1['Random']})", "Média Turnos": round(t1, 1)})

    # 2. qlearning vs random
    w2, t2 = run_matchup(QlearningAgent, "Q-learning", random_agent, "Random", MATCHES_PER_PAIR)
    results.append({"Confronto": "Q-learning vs Random", "Vitórias P1": f"Q-learning ({w2['Q-learning']})", "Vitórias P2": f"Random ({w2['Random']})", "Média Turnos": round(t2, 1)})

    # 3. rule-based vs stochastic
    w3, t3 = run_matchup(rule_agent, "Rule-Based", QlearningAgent, "Q-learning", MATCHES_PER_PAIR)
    results.append({"Confronto": "Rule-Based vs Q-learning", "Vitórias P1": f"Rule-Based ({w3['Rule-Based']})", "Vitórias P2": f"Q-learning ({w3['Q-learning']})", "Média Turnos": round(t3, 1)})

    df_res = pd.DataFrame(results)
    df_res.to_csv("resultado_comparativo_3_modelos.csv", index=False)

    print("\n📊 === RESULTADO DA COMPARAÇÃO DOS 3 MODELOS ===")
